...
> **현재 단계: Design Freeze 완료 → 개발 착수 단계.** 2026-06-26부로 설계가 동결([docs/DESIGN-FREEZE.md](docs/DESIGN-FREEZE.md), D-065)되었다. 코드는 아직 한 줄도 작성되지 않았다. D-077(ERD 동기화·API Sequence·Event Flow 완성)이 개발 착수 직전 최종 기술 문서를 완성한 데 이어, **D-078(Marketing Reward System 및 Lifestyle Wallet 구조 개선)로 Lifestyle 보상(기존 "+알파" 보너스)이 현금성 수당이 아니라 비현금성 Marketing Reward Program임을 데이터 구조로 명확화했다 — 새 MLM 정책이 아니며, 현금(Settlement)과 포인트(Lifestyle Wallet)는 절대 혼합하지 않는다.** D-072~D-077에서 반복된 "마지막 라운드" 표현의 신뢰도가 낮았던 점을 감안해 D-078을 마지막으로 단정하지 않는다. 전체 문서 지도는 **[docs/MASTER-INDEX.md](docs/MASTER-INDEX.md)** 를 가장 먼저 읽을 것.
...
- **의사결정 추적**: [DECISIONS.md](docs/DECISIONS.md)(D-001~D-078 확정 결정 + O-002~O-209 Open Decision, O-199 해소) · [CHANGELOG.md](docs/CHANGELOG.md) · [GAP-ANALYSIS.md](docs/GAP-ANALYSIS.md)(상용 ERP 대비 점검)
...
**155개+ 엔터티를 19개 도메인 클러스터(Mermaid `erDiagram` 25개)로 묶어 정리했다 — DATABASE.md와 100% 일치한다(D-077로 동기화 공백 해소, D-078로 클러스터19 추가).** D-069 쇼핑몰 운영/SEO 신규 테이블 18종은 클러스터14; D-072(`carts`/`cart_items`/`product_price_alerts`)/D-074(Board Engine 5종)/D-075(공제조합·E-Wallet·글로벌결제 6종)는 클러스터15~17; D-076(`admin_favorite_menus`/`saved_filters`/`notification_inbox_states`/`admin_notes`/`approval_delegations`)은 클러스터18; D-078(`reward_policies`)은 클러스터19. [ERD.md](docs/ERD.md) 참조 — 원본 텍스트 정의는 [DATABASE.md](docs/DATABASE.md) §3. Worker 중심 Event 처리 흐름은 **[EVENT-FLOW.md](docs/EVENT-FLOW.md)**(D-077, Mermaid Flowchart 10개)에 시각화되어 있다.
...
**D-072~D-077에서 반복된 "마지막 라운드" 표현의 신뢰도가 낮았던 점을 감안해, D-078도 마지막으로 단정하지 않는다. 이후 신규 요구사항은 Change Request(CR)로만 관리한다.**
