...
> **현재 단계: Design Freeze 완료 → 개발 착수 단계.** 2026-06-26부로 설계가 동결([docs/DESIGN-FREEZE.md](docs/DESIGN-FREEZE.md), D-065)되었다. 코드는 아직 한 줄도 작성되지 않았다. **D-076(ERP 운영 생산성 및 관리자 UX 완성)을 프로젝트의 마지막 운영 UX 설계 보강으로 지정하며, 설계·운영 준비·개발 준비가 모두 종료되었다. 이후 신규 요구사항은 Change Request(CR)로만 관리하고, 다음 단계는 Development Governance 문서 작업이다.** 전체 문서 지도는 **[docs/MASTER-INDEX.md](docs/MASTER-INDEX.md)** 를 가장 먼저 읽을 것.
...
- **의사결정 추적**: [DECISIONS.md](docs/DECISIONS.md)(D-001~D-076 확정 결정 + O-002~O-207 Open Decision, O-199 해소) · [CHANGELOG.md](docs/CHANGELOG.md) · [GAP-ANALYSIS.md](docs/GAP-ANALYSIS.md)(상용 ERP 대비 점검)
...
149개+ 엔터티를 17개 도메인 클러스터로 묶어 Mermaid ER 다이어그램으로 정리했다(D-069 쇼핑몰 운영/SEO 신규 테이블 18종은 클러스터14; D-072의 `carts`/`cart_items`/`product_price_alerts` 3종/D-074의 Board Engine 5종/D-075의 공제조합·E-Wallet·글로벌결제 6종은 클러스터15~17로 D-075 라운드에서 일괄 catch-up 반영 완료). **D-076의 신규 5테이블(`admin_favorite_menus` 등, DATABASE.md §3.62)은 본 ERD에 아직 미반영 — 1라운드 지연, 154개+ 엔터티 기준은 DATABASE.md가 최신.** [ERD.md](docs/ERD.md) 참조 — 원본 텍스트 정의는 [DATABASE.md](docs/DATABASE.md) §3.
...
✔ **ERP 운영 생산성 및 관리자 UX 완성(설계 마지막 라운드)** ([DECISIONS.md](docs/DECISIONS.md) D-076)
⬜ 개발 진행
⬜ QA
⬜ 운영
...
## ERP 운영 생산성 및 관리자 UX 완성 (D-076, 프로젝트의 마지막 운영 UX 설계 보강)
