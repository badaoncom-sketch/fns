...
- **의사결정 추적**: [DECISIONS.md](docs/DECISIONS.md)(D-001~D-074 확정 결정 + O-002~O-200 Open Decision) · [CHANGELOG.md](docs/CHANGELOG.md) · [GAP-ANALYSIS.md](docs/GAP-ANALYSIS.md)(상용 ERP 대비 점검)
...
REST 기준, 무거운 계산은 항상 `Job 생성 → 202 Accepted → 별도 상태조회` 패턴을 따른다(`api`에 계산 로직 금지). 28개 모듈의 엔드포인트 목록과 전역 컨벤션(페이지네이션/정렬/필터/에러포맷)은 [API-SPEC.md](docs/API-SPEC.md) 참조.

## ERD

143개+ 엔터티를 14개 도메인 클러스터로 묶어 Mermaid ER 다이어그램으로 정리했다(D-069 쇼핑몰 운영/SEO 신규 테이블 18종을 클러스터14로 D-070에서 반영 완료; D-072의 `carts`/`cart_items`/`product_price_alerts` 3종 및 D-074의 Board Engine 5종(`boards`/`board_posts` 등)은 다음 ERD 갱신 라운드에서 반영 필요). [ERD.md](docs/ERD.md) 참조 — 원본 텍스트 정의는 [DATABASE.md](docs/DATABASE.md) §3.
...
✔ **Dynamic Board Engine 설계 완료(설계 마지막 라운드)** ([DECISIONS.md](docs/DECISIONS.md) D-074)
⬜ 개발 진행
⬜ QA
⬜ 운영
...
**D-074를 끝으로 설계를 최종 종료하며, 이후 신규 요구사항은 Change Request(CR)로만 관리한다.**
