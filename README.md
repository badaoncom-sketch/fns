...
> **현재 단계: Design Freeze 완료 → 개발 착수 단계.** 2026-06-26부로 설계가 동결([docs/DESIGN-FREEZE.md](docs/DESIGN-FREEZE.md), D-065)되었다. 코드는 아직 한 줄도 작성되지 않았다. **D-070(쇼핑몰 운영 Phase 2 및 문서 동기화)을 마지막 설계 보강 라운드로 간주하며, 이후 신규 기능은 Change Request(CR)로만 관리한다.** 전체 문서 지도는 **[docs/MASTER-INDEX.md](docs/MASTER-INDEX.md)** 를 가장 먼저 읽을 것.
...
- **의사결정 추적**: [DECISIONS.md](docs/DECISIONS.md)(D-001~D-070 확정 결정 + O-002~O-195 Open Decision) · [CHANGELOG.md](docs/CHANGELOG.md) · [GAP-ANALYSIS.md](docs/GAP-ANALYSIS.md)(상용 ERP 대비 점검)
...
단일 통합 카탈로그(일반 쇼핑몰) + 회원몰(유지구매·정기배송·자동결제 센터). 패키지 상품도 별도 채널이 아니라 카탈로그 안의 상품 종류 중 하나다. 옵션/재고(LOT)/주문 병합·분리/부분교환/결제 재시도/리뷰/Bundle/검색/SEO·공유이미지 등 운영 고도화는 PRD §5.45~§5.46(D-069) 참조. 상세: [PRD.md](docs/PRD.md) §5.1.3, [SITEMAP.md](docs/SITEMAP.md).
...
REST 기준, 무거운 계산은 항상 `Job 생성 → 202 Accepted → 별도 상태조회` 패턴을 따른다(`api`에 계산 로직 금지). 25개 모듈의 엔드포인트 목록과 전역 컨벤션(페이지네이션/정렬/필터/에러포맷)은 [API-SPEC.md](docs/API-SPEC.md) 참조.

## ERD

135개+ 엔터티를 14개 도메인 클러스터로 묶어 Mermaid ER 다이어그램으로 정리했다(D-069 쇼핑몰 운영/SEO 신규 테이블 18종을 클러스터14로 D-070에서 반영 완료). [ERD.md](docs/ERD.md) 참조 — 원본 텍스트 정의는 [DATABASE.md](docs/DATABASE.md) §3.
...
✔ **쇼핑몰 운영/SEO 고도화 + 문서 동기화 완료(설계 마지막 라운드)** ([DECISIONS.md](docs/DECISIONS.md) D-069/D-070)
⬜ 개발 진행
⬜ QA
⬜ 운영
...
## 쇼핑몰 운영 / SEO / Analytics / Feed (D-069/D-070, 설계 마지막 보강)
...
상세는 [PRD.md](docs/PRD.md) §5.45~§5.50, 동기화된 산출물은 [API-SPEC.md](docs/API-SPEC.md) §2.21~§2.25 / [ERD.md](docs/ERD.md) 클러스터14 / [DATA-DICTIONARY.md](docs/DATA-DICTIONARY.md) §7 / [ROLE-MATRIX.md](docs/ROLE-MATRIX.md) §24~§25 / [TEST-PLAN.md](docs/TEST-PLAN.md) §2.10 참조. **D-070을 끝으로 설계를 종료하며, 이후 신규 기능은 Change Request(CR)로만 관리한다.**
