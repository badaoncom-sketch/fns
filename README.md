...
> **현재 단계: Design Freeze 완료 → 개발 착수 단계.** 2026-06-26부로 설계가 동결([docs/DESIGN-FREEZE.md](docs/DESIGN-FREEZE.md), D-065)되었다. 코드는 아직 한 줄도 작성되지 않았다. D-078(Marketing Reward System 및 Lifestyle Wallet 구조 개선)이 Lifestyle 보상(기존 "+알파" 보너스)을 비현금성 Marketing Reward Program으로 명확화한 데 이어, **D-079(Reward Policy 고도화 및 운영 시뮬레이션)로 Reward Formula가 append-only로 버전 관리되고(`reward_formula_versions`), Reward Simulation/Formula Test가 결과를 저장하지 않는 순수 계산으로 설계되었다 — 새 MLM 정책이 아니며, Settlement와 Lifestyle Wallet은 완전히 분리된 구조를 유지한다.** D-072~D-078에서 반복된 "마지막 라운드" 표현의 신뢰도가 낮았던 점을 감안해 D-079도 마지막으로 단정하지 않는다. 전체 문서 지도는 **[docs/MASTER-INDEX.md](docs/MASTER-INDEX.md)** 를 가장 먼저 읽을 것.
...
- **의사결정 추적**: [DECISIONS.md](docs/DECISIONS.md)(D-001~D-079 확정 결정 + O-002~O-210 Open Decision, O-199 해소) · [CHANGELOG.md](docs/CHANGELOG.md) · [GAP-ANALYSIS.md](docs/GAP-ANALYSIS.md)(상용 ERP 대비 점검)
...
Unilevel Sponsor Plan(추천조직 트리, LINE1~5). 후원수당·페어보너스·패키지 정책은 모두 관리자 설정 가능(하드코딩 없음). append-only 원장(`commission_records`)에 입력 스냅샷을 함께 저장해 사후 재현 가능. 상세: [COMPENSATION-RULES.md](docs/COMPENSATION-RULES.md), [DATABASE.md](docs/DATABASE.md) §3.5. **"+알파" 보너스(Lifestyle Bonus, §4.2)는 현금성 수당이 아니라 비현금성 Marketing Reward Program이다(D-078)** — Settlement는 Unilevel/제품판매수익/페어보너스(현금성 3종)만 처리하며, Lifestyle 보상은 `reward_policies`→Lifestyle Wallet 구조로 별도 처리한다. 상세: §5.83~§5.86.
...
**156개+ 엔터티를 20개 도메인 클러스터(Mermaid `erDiagram` 26개)로 묶어 정리했다 — DATABASE.md와 100% 일치한다(D-077로 동기화 공백 해소, D-078/D-079로 클러스터19~20 추가).** D-069 쇼핑몰 운영/SEO 신규 테이블 18종은 클러스터14; D-072(`carts`/`cart_items`/`product_price_alerts`)/D-074(Board Engine 5종)/D-075(공제조합·E-Wallet·글로벌결제 6종)는 클러스터15~17; D-076(`admin_favorite_menus`/`saved_filters`/`notification_inbox_states`/`admin_notes`/`approval_delegations`)은 클러스터18; D-078(`reward_policies`)은 클러스터19; D-079(`reward_formula_versions`)는 클러스터20. [ERD.md](docs/ERD.md) 참조 — 원본 텍스트 정의는 [DATABASE.md](docs/DATABASE.md) §3. Worker 중심 Event 처리 흐름은 **[EVENT-FLOW.md](docs/EVENT-FLOW.md)**(D-077, Mermaid Flowchart 10개)에 시각화되어 있다.
...
✔ **Marketing Reward System 및 Lifestyle Wallet 구조 개선** ([DECISIONS.md](docs/DECISIONS.md) D-078)
✔ **Reward Policy 고도화 및 운영 시뮬레이션** ([DECISIONS.md](docs/DECISIONS.md) D-079)
⬜ 개발 진행
⬜ QA
⬜ 운영
...
## ERD 동기화·API Sequence·Event Flow 완성 (D-077)
...
## Marketing Reward System 및 Lifestyle Wallet 구조 개선 (D-078)

**새 MLM 정책이 아니다.** Lifestyle 보상(기존 "+알파" 보너스: 여행/자동차/자기계발)이 현금성 수당이 아니라 비현금성 **Marketing Reward Program**임을 데이터 구조로 명확히 한다 — `Reward Policy → Lifestyle Point Ledger → Lifestyle Wallet → 사용` 구조를 표준화했다. **현금(Settlement/CASH E-Wallet)과 포인트(Lifestyle Point)는 절대 혼합하지 않는다.**

- **Reward Policy**: Marketing Program Engine 산하 신규 테이블 `reward_policies` 1종 — 적립기준/적립방식/적립률/누적방식/누적기간/최소조건/최대한도/사용가능기간 등을 관리자가 직접 설정(하드코딩 없음). 기존 Travel/Car/자기계발 3종의 수치는 `plan_definition.lifestyle_bonus`(D-032)를 그대로 참조하며 중복 저장하지 않음 — Golf 등 신규 Program만 새 수치를 입력.
- **Lifestyle Wallet**: 신규 테이블 없이 기존 E-Wallet(`member_wallets`/`wallet_transactions`, D-075)을 확장 재사용 — `wallet_type`(CASH/LIFESTYLE_POINT) 컬럼 추가, `counts_toward_compliance_limit`(35% 한도 산정 포함 여부) 컬럼 추가, `transaction_type`에 RESTORE/EXPIRE 허용값 추가. Lifestyle Point는 출금(은행송금) 대상이 아니며 쇼핑몰/Program 신청 사용으로만 차감된다.
- **라우팅 재조정**: 기존 `lifestyle_bonus_accumulations`(§3.25)의 적립 산정 로직 자체는 변경하지 않는다 — D-041이 정했던 `point_transactions` 경유 라우팅을 본 라운드부터 `wallet_transactions`(wallet_type=LIFESTYLE_POINT)로 재조정했을 뿐이다.
- **Settlement와의 경계**: Settlement(Unilevel Sponsor Bonus/Product Sales Bonus/Pair Bonus, 현금성 3종)는 본 라운드로 전혀 변경되지 않는다.

상세는 [PRD.md](docs/PRD.md) §5.83~§5.86, 동기화된 산출물은 [DATABASE.md](docs/DATABASE.md) §3.63 / [WIREFRAME.md](docs/WIREFRAME.md) / [API-SPEC.md](docs/API-SPEC.md) §2.39 / [DATA-DICTIONARY.md](docs/DATA-DICTIONARY.md) §13 / [ERD.md](docs/ERD.md) 클러스터19 / [ROLE-MATRIX.md](docs/ROLE-MATRIX.md) §34 / [TEST-PLAN.md](docs/TEST-PLAN.md) §2.17 참조. 신규 Open Decision은 **O-208(쇼핑몰 사용 기본값)/O-209(과거 적립분 마이그레이션 여부) 2건**이다. 신규 Business Rule 없음, 새 MLM 정책 없음.

## Reward Policy 고도화 및 운영 시뮬레이션 (D-079)

Marketing Reward System(D-078)을 완성하는 보강 — **새 MLM 정책이 아니다.** Reward Policy 안에 **Reward Formula**를 추가하고 append-only로 버전 관리하며, 저장하지 않는 Simulation/Test 기능을 제공한다.

- **Reward Formula / Formula Version**: 신규 테이블 `reward_formula_versions` 1종(append-only) — 매출×%/PV/BV/고정Point/관리자 정의 계산식을 관리자가 직접 설정. 정책 수정 시 기존 Formula 행을 수정하지 않고 새 버전을 생성하며, 기존 버전은 조회만 가능(append-only). 정책당 활성 버전은 항상 정확히 1개. `reward_policies`의 기존 `accrual_basis`/`accrual_method`/`accrual_rate`는 활성 버전의 파생 캐시로 의미가 바뀐다.
- **Reward Simulation**: 관리자가 매출/PV/BV 등 가상 입력값으로 기존 정책의 예상 Point를 미리 계산한다. **결과는 저장하지 않는다** — 순수 계산이며, 실행 메타데이터만 기존 `audit_logs`에 기록한다.
- **Formula Test**: 새 Formula Version을 저장하기 전에 미리 테스트한다. Simulation과 동일한 계산 로직·동일한 비영속성 원칙을 재사용(신규 계산 엔진 없음).
- **Reward History**: Formula 변경 이력은 `reward_formula_versions` 조회, Policy/Simulation/Test 실행 이력은 기존 `audit_logs` 조회 — 신규 이력 테이블 없음.
- **Settlement와의 경계**: Settlement는 본 라운드로 전혀 변경되지 않는다 — Reward Formula/Simulation/Test 무엇도 `settlement_batches`/`settlement_items`/`commission_records`를 읽거나 쓰지 않는다.

상세는 [PRD.md](docs/PRD.md) §5.87~§5.90, 동기화된 산출물은 [DATABASE.md](docs/DATABASE.md) §3.64 / [WIREFRAME.md](docs/WIREFRAME.md) / [API-SPEC.md](docs/API-SPEC.md) §2.39 보강 / [DATA-DICTIONARY.md](docs/DATA-DICTIONARY.md) §14 / [ERD.md](docs/ERD.md) 클러스터20 / [TEST-PLAN.md](docs/TEST-PLAN.md) §2.18 참조(ROLE-MATRIX.md는 신규 권한 복잡도 없어 대상 제외). 신규 Open Decision은 **O-210(Custom Formula 표현·안전한 평가 방식 미확정 — 결정 전까지 CUSTOM 타입 비활성화 권고) 1건**이다. 신규 Business Rule 없음, 새 MLM 정책 없음.

**D-072~D-078에서 반복된 "마지막 라운드" 표현의 신뢰도가 낮았던 점을 감안해, D-079도 마지막으로 단정하지 않는다. 이후 신규 요구사항은 Change Request(CR)로만 관리한다.**
