...
> 상태: v1.7 ([DECISIONS.md](DECISIONS.md) D-077 — ERD 동기화·API Sequence·Event Flow 완성: 신규 기능 없음, 순수 문서 정리 라운드. [ERD.md](ERD.md)에 클러스터18(§3.62, D-076의 신규 5테이블)을 추가해 **DATABASE.md와 100% 일치**(18클러스터/24다이어그램/154엔터티) 달성. 신규 문서 **[API-SEQUENCE.md](API-SEQUENCE.md)**(19개 Mermaid Sequence Diagram)/**[EVENT-FLOW.md](EVENT-FLOW.md)**(10개 Mermaid Flowchart, EVENT-CATALOG.md 미등재 이벤트명 15건 명시 구분) 작성. Database↔ERD↔API↔Event Flow↔Sequence↔State Machine↔Business Rule 7개 문서 간 Source of Truth 교차검증 — **충돌 0건.** 신규 Business Rule/테이블/Engine/Open Decision 모두 없음(0건). **사용자가 본 작업 완료 후 Development Governance 단계로 진행한다고 명시.** D-076 — ERP 운영 생산성 및 관리자 UX 완성: PRD.md §5.71~§5.82 신설, 신규 테이블 5종, O-199 해소, O-206~O-207 등록) · 최종 수정일: 2026-06-27 · 단계: **Design Freeze 완료 → 개발 착수**
...
| [PRD.md](PRD.md) | 기능 요구사항(§5.1~§5.82, 82개 절) | Draft v0.33 | D-076 |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 시스템 아키텍처(5서비스 구조, API 설계원칙, 컴플라이언스 엔진) | Draft v0.16 | D-062 |
| [DATABASE.md](DATABASE.md) | 데이터 모델 개념 설계(§3, 154개+ 엔터티) — ERD와 100% 일치 확인(D-077) | Draft v0.30 | D-077 |
...
| [DECISIONS.md](DECISIONS.md) | 의사결정 기록 — 확정(D-001~D-077) + Open Decision(O-002~O-207, O-199 해소) + 개발 Blocker 목록(§2.2) | Living document | D-077 |
| [CHANGELOG.md](CHANGELOG.md) | 문서 체계 변경 이력(Keep a Changelog 형식) | Living document | v2.3.0 |
...
| [ROLE-MATRIX.md](ROLE-MATRIX.md) | 역할×액션×모듈 권한 매트릭스 + §27 Board Engine + §28~§30 공제조합/E-Wallet/글로벌결제 관리 + §32 ERP 운영 생산성 | v0.6 | D-076 |
| [ERD.md](ERD.md) | DATABASE.md의 시각화(Mermaid ER 다이어그램) + 클러스터15~18(D-072/D-074/D-075/D-076 모두 반영) — **DATABASE.md와 100% 일치(18클러스터/24다이어그램/154엔터티)** | v0.4 | D-077 |
| [API-SPEC.md](API-SPEC.md) | REST API 명세(전역 컨벤션 + 모듈별 엔드포인트) + §2.21~§2.38 쇼핑몰/SEO/Feed/Cart/AdminTaskQueue/Board Engine/공제조합/E-Wallet/글로벌결제/ERP 운영생산성 + API-SEQUENCE.md 교차참조 | v0.8 | D-077 |
...
| [WIREFRAME.md](WIREFRAME.md) | 화면 레이아웃 아키타입 및 모듈별 매핑 + §4 관리자 설정 운영 예시 + 장바구니·운영자대시보드·업무Queue·Timeline·QuickAction 화면 + Board Engine 모듈 + ERP 운영생산성 모듈 | v0.7 | D-076 |
...
| [TEST-PLAN.md](TEST-PLAN.md) | 테스트 전략(Unit/Integration/E2E + §2.10~§2.15 쇼핑몰 Phase2/SEO·UX·알림·운영UX·Board Engine·공제조합/E-Wallet/글로벌결제·ERP운영생산성 테스트) | v0.8 | D-076 |
...
| [DATA-DICTIONARY.md](DATA-DICTIONARY.md) | DATABASE.md 기반 컬럼 사전 + §7~§9 쇼핑몰 Phase2/SEO·UX·대시보드·Board Engine + §11 공제조합/E-Wallet/글로벌결제 + §12 ERP 운영생산성 | v0.6 | D-076 |
| [MASTER-INDEX.md](MASTER-INDEX.md) | 전체 문서 색인(문서 목적·상태·의존관계·읽는 순서·개발 준비도) | v1.7 | D-077 |
...
- ✔ **ERD 동기화·API Sequence·Event Flow 완성(개발 착수 직전 최종 기술 문서)** — ERD.md가 DATABASE.md와 100% 일치(18클러스터/154엔터티), [API-SEQUENCE.md](API-SEQUENCE.md)/[EVENT-FLOW.md](EVENT-FLOW.md) 신규 작성, Source of Truth 교차검증 충돌 0건([DECISIONS.md](DECISIONS.md) D-077) — 신규 기능/테이블/Business Rule/Open Decision 없음
