...
> 상태: v1.5 ([DECISIONS.md](DECISIONS.md) D-075 — 한국 공제조합 연동·E-Wallet·글로벌 결제: PRD.md §5.68~§5.70 신설(**전부 Tenant별 선택 기능**) + DATABASE.md §3.59~§3.61(`compliance_member_registrations`/`compliance_transmission_items`/`member_wallets`/`wallet_transactions`/`wallet_withdrawal_requests`/`payment_webhook_events` 신규 6테이블, E-Wallet은 append-only Ledger) + STATE-MACHINE/API-SPEC/DATA-DICTIONARY/ERD/ROLE-MATRIX/TEST-PLAN 6개 문서 동기화(ERD는 D-072/D-074분까지 일괄 catch-up). **MLM 보상플랜·정산 계산 로직·기존 Business Rule·ERP Core 구조 변경 없음**, 신규 Open Decision O-201~O-205(5건). **본 라운드를 끝으로 설계를 최종 종료한다.**) · 최종 수정일: 2026-06-26 · 단계: **Design Freeze 완료 → 개발 착수**
...
| [PRD.md](PRD.md) | 기능 요구사항(§5.1~§5.70, 70개 절) | Draft v0.32 | D-075 |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 시스템 아키텍처(5서비스 구조, API 설계원칙, 컴플라이언스 엔진) | Draft v0.16 | D-062 |
| [DATABASE.md](DATABASE.md) | 데이터 모델 개념 설계(§3, 149개+ 엔터티) | Draft v0.28 | D-075 |
...
| [DECISIONS.md](DECISIONS.md) | 의사결정 기록 — 확정(D-001~D-075) + Open Decision(O-002~O-205) + 개발 Blocker 목록(§2.2) | Living document | D-075 |
| [CHANGELOG.md](CHANGELOG.md) | 문서 체계 변경 이력(Keep a Changelog 형식) | Living document | v2.1.0 |
...
| [ROLE-MATRIX.md](ROLE-MATRIX.md) | 역할×액션×모듈 권한 매트릭스 + §27 Board Engine + §28~§30 공제조합/E-Wallet/글로벌결제 관리 | v0.5 | D-075 |
| [ERD.md](ERD.md) | DATABASE.md의 시각화(Mermaid ER 다이어그램) + 클러스터15~17(D-072/D-074/D-075 일괄 catch-up) | v0.3 | D-075 |
| [API-SPEC.md](API-SPEC.md) | REST API 명세(전역 컨벤션 + 모듈별 엔드포인트) + §2.21~§2.31 쇼핑몰/SEO/Feed/Cart/AdminTaskQueue/Board Engine/공제조합/E-Wallet/글로벌결제 | v0.6 | D-075 |
...
| [TEST-PLAN.md](TEST-PLAN.md) | 테스트 전략(Unit/Integration/E2E + §2.10~§2.14 쇼핑몰 Phase2/SEO·UX·알림·운영UX·Board Engine·공제조합/E-Wallet/글로벌결제 테스트) | v0.7 | D-075 |
...
| [DATA-DICTIONARY.md](DATA-DICTIONARY.md) | DATABASE.md 기반 컬럼 사전 + §7~§9 쇼핑몰 Phase2/SEO·UX·대시보드·Board Engine + §11 공제조합/E-Wallet/글로벌결제 | v0.5 | D-075 |
| [MASTER-INDEX.md](MASTER-INDEX.md) | 전체 문서 색인(문서 목적·상태·의존관계·읽는 순서·개발 준비도) | v1.5 | D-075 |
