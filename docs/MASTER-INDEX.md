...
> 상태: v1.6 ([DECISIONS.md](DECISIONS.md) D-076 — ERP 운영 생산성 및 관리자 UX 완성: PRD.md §5.71~§5.82 신설(Global Search/Approval Center/Favorite Menu·Saved Filter/Recent Activity/Notification Inbox/Tenant Usage Dashboard/Approval History/Personal Workspace/Operator Notes/관리자 Dashboard 보강/Command Palette/Universal Clipboard) + DATABASE.md §3.62(`admin_favorite_menus`/`saved_filters`/`notification_inbox_states`/`admin_notes`/`approval_delegations` 신규 5테이블) + WIREFRAME/API-SPEC/DATA-DICTIONARY/ROLE-MATRIX/TEST-PLAN 5개 문서 동기화(ERD는 본 라운드 미반영, 1라운드 지연). **신규 Engine 없음, MLM·Settlement·ERP Core·Workflow·쇼핑몰 구조 변경 없음**, **O-199 해소**, 신규 Open Decision O-206~O-207(2건). **사용자가 본 라운드를 프로젝트의 마지막 운영 UX 설계 보강으로 지정, 다음 단계는 Development Governance 문서 작업.** D-075 — 한국 공제조합 연동·E-Wallet·글로벌 결제: PRD.md §5.68~§5.70 신설(전부 Tenant별 선택 기능), 신규 Open Decision O-201~O-205(5건)) · 최종 수정일: 2026-06-27 · 단계: **Design Freeze 완료 → 개발 착수**
...
| [DATABASE.md](DATABASE.md) | 데이터 모델 개념 설계(§3, 154개+ 엔터티) | Draft v0.29 | D-076 |
...
| [DECISIONS.md](DECISIONS.md) | 의사결정 기록 — 확정(D-001~D-076) + Open Decision(O-002~O-207, O-199 해소) + 개발 Blocker 목록(§2.2) | Living document | D-076 |
| [CHANGELOG.md](CHANGELOG.md) | 문서 체계 변경 이력(Keep a Changelog 형식) | Living document | v2.2.0 |
...
| [ERD.md](ERD.md) | DATABASE.md의 시각화(Mermaid ER 다이어그램) + 클러스터15~17(D-072/D-074/D-075 일괄 catch-up) — **D-076(§3.62) 5테이블 미반영, 1라운드 지연** | v0.3 | D-075 |
| [API-SPEC.md](API-SPEC.md) | REST API 명세(전역 컨벤션 + 모듈별 엔드포인트) + §2.21~§2.38 쇼핑몰/SEO/Feed/Cart/AdminTaskQueue/Board Engine/공제조합/E-Wallet/글로벌결제/ERP 운영생산성 | v0.7 | D-076 |
...
| [MASTER-INDEX.md](MASTER-INDEX.md) | 전체 문서 색인(문서 목적·상태·의존관계·읽는 순서·개발 준비도) | v1.6 | D-076 |
