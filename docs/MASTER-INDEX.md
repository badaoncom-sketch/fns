...
> 상태: v1.8 ([DECISIONS.md](DECISIONS.md) D-078 — Marketing Reward System 및 Lifestyle Wallet 구조 개선: **새 MLM 정책이 아니다** — Lifestyle 보상(기존 "+알파" 보너스)이 현금성 수당이 아니라 비현금성 Marketing Reward Program임을 데이터 구조로 명확화한다. `Reward Policy → Lifestyle Point Ledger → Lifestyle Wallet → 사용` 구조 표준화, **현금(Settlement/CASH E-Wallet)과 포인트 절대 비혼합**. [DATABASE.md](DATABASE.md) §3.63에 `reward_policies`(신규 1종) + 기존 E-Wallet(§3.60) 확장(`member_wallets.wallet_type`/`wallet_transactions.counts_toward_compliance_limit` 컬럼, RESTORE/EXPIRE 허용값). D-041의 Lifestyle Bonus 적립 라우팅을 `point_transactions`에서 `wallet_transactions`로 재조정(산정 로직 자체는 무변경). PRD/WIREFRAME/API-SPEC/DATA-DICTIONARY/ERD(클러스터19)/ROLE-MATRIX/TEST-PLAN 7개 문서 동기화. 신규 Business Rule/Engine 없음, O-208~O-209(2건) 등록. **D-077이 "본 라운드를 끝으로 최종 기술 문서를 완성"이라고 명시했음에도 D-078이 추가 요청되었다 — D-072~D-077에서 반복된 "마지막 라운드" 표현의 신뢰도가 낮음을 감안해 본 절은 D-078을 마지막으로 단정하지 않는다.** D-077 — ERD 동기화·API Sequence·Event Flow 완성: ERD.md 클러스터18 추가로 DATABASE.md와 100% 일치(당시 18클러스터/24다이어그램/154엔터티) 달성, 신규 문서 API-SEQUENCE.md/EVENT-FLOW.md 작성, Source of Truth 교차검증 충돌 0건) · 최종 수정일: 2026-06-27 · 단계: **Design Freeze 완료 → 개발 착수**
...
| [PRD.md](PRD.md) | 기능 요구사항(§5.1~§5.86, 86개 절) | Draft v0.34 | D-078 |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 시스템 아키텍처(5서비스 구조, API 설계원칙, 컴플라이언스 엔진) | Draft v0.16 | D-062 |
| [DATABASE.md](DATABASE.md) | 데이터 모델 개념 설계(§3, 155개+ 엔터티) — ERD와 100% 일치(D-077), §3.63 Reward Policy/Lifestyle Wallet(D-078) | Draft v0.31 | D-078 |
...
| [DECISIONS.md](DECISIONS.md) | 의사결정 기록 — 확정(D-001~D-078) + Open Decision(O-002~O-209, O-199 해소) + 개발 Blocker 목록(§2.2) | Living document | D-078 |
| [CHANGELOG.md](CHANGELOG.md) | 문서 체계 변경 이력(Keep a Changelog 형식) | Living document | v2.4.0 |
...
| [ERD.md](ERD.md) | DATABASE.md의 시각화(Mermaid ER 다이어그램) + 클러스터15~19(D-072/D-074/D-075/D-076/D-078 모두 반영) — **DATABASE.md와 100% 일치(19클러스터/25다이어그램/155엔터티)** | v0.5 | D-078 |
| [API-SPEC.md](API-SPEC.md) | REST API 명세(전역 컨벤션 + 모듈별 엔드포인트) + §2.21~§2.39 쇼핑몰/SEO/Feed/Cart/AdminTaskQueue/Board Engine/공제조합/E-Wallet/글로벌결제/ERP 운영생산성/Marketing Reward + API-SEQUENCE.md 교차참조 | v0.9 | D-078 |
...
| [WIREFRAME.md](WIREFRAME.md) | 화면 레이아웃 아키타입 및 모듈별 매핑 + §4 관리자 설정 운영 예시 + 장바구니·운영자대시보드·업무Queue·Timeline·QuickAction 화면 + Board Engine 모듈 + ERP 운영생산성 모듈 + Reward Policy/Lifestyle Wallet 화면 | v0.8 | D-078 |
...
| [TEST-PLAN.md](TEST-PLAN.md) | 테스트 전략(Unit/Integration/E2E + §2.10~§2.17 쇼핑몰 Phase2/SEO·UX·알림·운영UX·Board Engine·공제조합/E-Wallet/글로벌결제·ERP운영생산성·Marketing Reward 테스트) | v0.9 | D-078 |
...
| [DATA-DICTIONARY.md](DATA-DICTIONARY.md) | DATABASE.md 기반 컬럼 사전 + §7~§9 쇼핑몰 Phase2/SEO·UX·대시보드·Board Engine + §11 공제조합/E-Wallet/글로벌결제 + §12 ERP 운영생산성 + §13 Marketing Reward/Lifestyle Wallet | v0.7 | D-078 |
| [MASTER-INDEX.md](MASTER-INDEX.md) | 전체 문서 색인(문서 목적·상태·의존관계·읽는 순서·개발 준비도) | v1.8 | D-078 |
