...
> §2.39는 [DECISIONS.md](DECISIONS.md) D-078 참조. 신규 Business Rule 없음, 신규 MLM 정책 없음. §2.5 Compensation/Settlement는 본 절에서 전혀 변경하지 않으며, Lifestyle Point는 어떤 엔드포인트에서도 현금 Settlement와 합산·교환되지 않는다. 신규 Open Decision은 **O-208(쇼핑몰 사용 기본값)/O-209(과거 적립분 마이그레이션 여부)** 2건이며 모두 DATABASE.md §3.63/PRD.md §5.83~§5.86 설계 시 이미 등록됨 — 본 절에서 신규로 추가하지 않음.
