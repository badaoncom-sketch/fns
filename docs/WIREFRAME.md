...
> 상태: v0.4 ([DECISIONS.md](DECISIONS.md) D-072 — 쇼핑몰 UX·알림·운영자 대시보드 완성: §2 매핑표에 장바구니 상세/상품비교·가격인하알림/배송추적/관리자업무Queue/운영자대시보드/알림템플릿 상세 화면 추가. D-069 — 쇼핑몰 운영 고도화/SEO: §2 매핑표에 옵션재고고도화/SEO/Bundle/주문병합분리/배송보강 화면 신규 추가) · 최종 수정일: 2026-06-26 · 단계: 설계(Design)
...
| | 운영자 대시보드 (§5.59, 신규) | A4 (자유구성형, 기본 템플릿 제공) | 3개 탭(오늘 지표/긴급 처리 지표/쇼핑몰 운영 지표) — 긴급 처리 지표 카드는 [관리자 업무 Queue](#) 화면으로 딥링크, 신규 위젯 종류만 추가(엔진 변경 없음) |
...
| | 장바구니 상세(§5.58.2, 신규 `carts`/`cart_items`) | A6-3 확장 | 옵션·수량 변경 인라인, 선택삭제, 품절/가격변경 배지(파생), 배송비예상·쿠폰적용가능여부 표시(파생), [관심상품으로 이동]/[나중에 구매하기](O-197) |
