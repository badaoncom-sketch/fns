...
> 상태: Draft v0.35 (D-079 — Reward Policy 고도화 및 운영 시뮬레이션: §5.87~§5.90 신규(Reward Formula/Formula Version/Reward Simulation/Formula Test/Reward Dashboard·History). Marketing Reward System(D-078)을 완성하는 보강 — **새 MLM 정책 아님.** Formula는 append-only 버전 관리(`reward_formula_versions`, 신규 1종), Simulation/Test는 신규 테이블 없이 계산만 수행(결과 미저장). Settlement·ERP Core·Workflow 구조 변경 없음, 신규 Open Decision O-210(1건). D-078 — Marketing Reward System 및 Lifestyle Wallet 구조 개선: §5.83~§5.86 신규(Reward Policy/Lifestyle Point·Wallet/Program 신청 사용/관리자 화면). **새 MLM 정책 아님** — Lifestyle 보상(기존 "+알파" 보너스)의 저장 위치를 `point_transactions`(D-041)에서 기존 E-Wallet 구조 확장(`wallet_type=LIFESTYLE_POINT`)으로 재라우팅해 현금(Settlement)과 포인트를 명확히 분리. 신규 Database 테이블 1종(`reward_policies`)뿐, 신규 Open Decision O-208~O-209(2건). D-076 — ERP 운영 생산성 및 관리자 UX 완성: §5.71~§5.82 신규(Global Search/Approval Center/Favorite Menu·Saved Filter/Recent Activity/Notification Inbox/Tenant Usage Dashboard/Approval History/Personal Workspace/Operator Notes/관리자 Dashboard 보강/Command Palette/Universal Clipboard) + §5.61 보강(Quick Action 확장, **O-199 해소**). 신규 Engine 없음, 신규 Business Rule 없음, MLM·Settlement·ERP Core·Workflow·쇼핑몰 구조 변경 없음. 신규 Database 테이블 5종(`admin_favorite_menus`/`saved_filters`/`notification_inbox_states`/`admin_notes`/`approval_delegations`)뿐, 신규 Open Decision O-206~O-207(2건). D-075 — 한국 공제조합 연동·E-Wallet·글로벌 결제: §5.68~§5.70 신규, 전부 Tenant별 선택 기능) · 최종 수정일: 2026-06-27 · 단계: 설계(Design)
...
### 5.87 Reward Formula / Formula Version (New Feature — [DECISIONS.md](DECISIONS.md) D-079, Marketing Reward System 완성 보강 — 새 MLM 정책 아님)

> Reward Policy(§5.83) 안에 **Reward Formula**를 추가한다 — 매출×0.2%↓Point, PV↓Point, BV↓Point, 또는 관리자 정의 계산식 등을 관리자가 직접 설정한다(하드코딩 없음).

| 항목 | 설명 |
|---|---|
| Formula 유형 | 매출 비율(%)/PV 기준/BV 기준/고정 Point/관리자 정의 계산식 — 자유 확장값 |
| Formula 정의 | 관리자 설정 JSON(`plan_definition`, D-032와 동일한 패턴) |
| 적립률 | Formula 버전에 종속된 수치 |

**Formula Version 관리**: Reward Formula는 append-only로 버전 관리한다. **Reward Policy 변경 시 기존 Formula를 수정하지 않고 새 Version을 생성한다 — 기존 Version은 조회만 가능하다.** 정책당 항상 정확히 1개의 활성(active) 버전만 존재한다.

- 관리자 정의 계산식(Custom Formula)의 정확한 표현 방식(자유 수식 문자열 vs 구조화된 규칙) 및 안전한 평가 메커니즘은 미확정 — **O-210**.
- 상세 데이터 모델: [DATABASE.md](DATABASE.md) §3.64 `reward_formula_versions`.

### 5.88 Reward Simulation (New Feature — [DECISIONS.md](DECISIONS.md) D-079, 저장하지 않는 순수 계산)

> 관리자가 Reward Policy를 **수정하기 전에** 가상의 입력값(예: 매출 100만원, PV 500)으로 예상 Point를 미리 계산해 본다. **Simulation 결과는 저장하지 않는다 — 순수 계산만 수행한다.** 신규 테이블 없음, 실행 이력(요청자/시각/입력값)만 `audit_logs`(기존)에 기록하고 계산 결과는 어떤 테이블에도 남기지 않는다.

### 5.89 Formula Test (New Feature — [DECISIONS.md](DECISIONS.md) D-079, 저장하지 않는 순수 계산)

> 관리자가 새 Formula Version을 **저장하기 전에** 입력값→예상 결과를 미리 테스트한다. 실제 데이터(`wallet_transactions`/`reward_formula_versions` 등)는 전혀 변경하지 않는다. §5.88 Simulation과 동일한 계산 로직을 재사용하며 신규 계산 엔진을 만들지 않는다 — Simulation은 "이미 등록된 정책"을, Formula Test는 "아직 저장하지 않은 Formula 초안"을 대상으로 한다는 점만 다르다.

### 5.90 Reward Dashboard / Reward History (New Feature — [DECISIONS.md](DECISIONS.md) D-079)

Dashboard에 Program별 적립/사용/잔액, Formula별 사용 현황, Wallet 현황 위젯 추가를 검토한다(Dashboard Builder 재사용, 신규 테이블 없음).

| 이력 | 조회 방법 |
|---|---|
| Formula 변경 이력 | `reward_formula_versions`(§3.64)를 정책별 시간순 조회 |
| Policy 변경 이력 | `audit_logs`(기존, 관리자 설정 변경 추적 패턴 재사용) |
| Simulation 실행 이력 / Formula Test 실행 이력 | `audit_logs`(기존) — 계산 결과 자체는 저장하지 않으므로 실행 메타데이터(요청자/시각/입력값)만 조회 가능 |

> §5.87~§5.90은 [DECISIONS.md](DECISIONS.md) D-079(Reward Policy 고도화 및 운영 시뮬레이션) 참조. **신규 Business Rule 없음. 새 MLM 정책 없음. Settlement·ERP Core·Workflow 구조 변경 없음.** 신규 Database 테이블은 `reward_formula_versions` 1종뿐이며, Simulation/Formula Test는 신규 테이블 없이 계산만 수행한다. 신규 Open Decision은 **O-210(Custom Formula 표현·평가 방식) 1건**이다.
